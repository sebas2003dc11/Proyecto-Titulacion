...
<div class="form-group row col-12 px-4">
    <div class="col-12 col-sm-9">
        <input type="text" id="dni" class="form-control p-3" placeholder="DNI / Nombre / Razon Social del Remitente">
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="formularioRemitente" action="{{ route('remitente.store') }}" method="POST" onsubmit="return validarFormulario()">
                    @csrf

                    <div class="fl-flex-label mb-4 px-2 col-12">
                        <input required type="text" placeholder="DNI" class="input input__text" name="dni"
                            value="{{ old('dni') }}">
                        <span id="errorDni" class="text-danger font-weight-bold">@error('dni'){{ $message }}@enderror</span> <!-- Mensaje de error para DNI -->
                    </div>
                    <div class="fl-flex-label mb-4 px-2 col-12">
                        <input required type="text" placeholder="Nombre o Razon Social" class="input input__text"
                            name="nombre" value="{{ old('nombre') }}">
                        <span id="errorNombre" class="text-danger font-weight-bold">@error('nombre'){{ $message }}@enderror</span> <!-- Mensaje de error para Nombre -->
                    </div>
                    ...
                    <div class="fl-flex-label mb-4 px-2 col-12">
                        <input required type="text" placeholder="Teléfono" class="input input__text" name="telefono"
                            value="{{ old('telefono') }}">
                        <span id="errorTelefono" class="text-danger font-weight-bold">@error('telefono'){{ $message }}@enderror</span> <!-- Mensaje de error para Teléfono -->
                    </div>

                    <div class="text-right p-2">
                        <a data-dismiss="modal" class="btn btn-secondary btn-rounded" onclick="limpiarFormulario()">Atras</a>
...
<script>
    ...
    function validarFormulario() {
        let formValido = true; // Variable para controlar si el formulario es válido
        ...
        if (dni.length < 8 || dni.length > 20) {
            ...
            dniError.textContent = ''; // Limpiar mensaje de error si es válido
        }

        // Validación del Teléfono
        let telefono = document.getElementsByName('telefono')[0].value;
        let telefonoError = document.getElementById('errorTelefono');
        let telefonoRegex = /^[0-9]{9,}$/; // Expresión regular para al menos 9 dígitos
        if (!telefonoRegex.test(telefono)) {
            telefonoError.textContent = 'El teléfono debe contener solo números y tener al menos 9 dígitos.';
            formValido = false; // El formulario no es válido
        } else {
            telefonoError.textContent = ''; // Limpiar mensaje de error si es válido
        }

        return formValido; // Devuelve true si el formulario es válido, false si no lo es
    }

    function limpiarFormulario() {
        document.getElementById("formularioRemitente").reset(); // Resetea el formulario
